...
A family disclosure declared after the results and before this
manuscript (`analysis/EVENT_FAMILY_ADDENDUM.md` and its correction)
reports the arithmetic from the committed ranks. Bonferroni across the
three unit tests, valid under any dependence among them, gives Rhode
Island an adjusted strict p of 3 × 1/43 = 0.070; Holm returns the same
rejection for Rhode Island and stops at Kentucky (0.302 against 0.05).
A six-test family that adds the decomposition's three channel tests
has a threshold of 0.10/6 = 0.017, below every floor the pools can
reach (1/43 and 1/35), so no result in this paper could clear it; the
channel tests condition on Rhode Island's signal and ask a second
question, and carry their own three-way adjustment in their frozen
protocol. Rhode Island and Kentucky share one donor pool and panel, and
