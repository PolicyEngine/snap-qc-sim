...
| K4 | Engine-vs-Minimodel divergence catalog (served in the deployed simulator's engine view): for each of the seven verified states, BENFIX reconciles with the recorded error amount exactly (weighted concordance 1.0) while FSBEN concordance runs 0.802 (CA) to 0.915 (GA); 881 of 6,081 replayed cases diverge, partitioned by file coding into allotment_adjustment (164), error_correction_arithmetic (457), and recorded_correct_nonformula (260). National weighted concordance over all 44,800 CASE==1 records: BENFIX 0.99997, FSBEN 0.8364. Payload sha 2517d26e… pinned in app.js and browser-verified | analysis/engine_comparison.json (hash-locked by tests/test_engine_comparison.py; suites vendored from axiom-oracles@f30f9a10 under paper/snapshot/oracle-suites/) |
