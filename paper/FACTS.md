...
| A2 | The FY2028 share is keyed to the state's FY2025 or FY2026 rate (state election); FY2029 onward uses the third preceding fiscal year | 7 U.S.C. 2013(a)(2)(B)(i)–(ii) |
| A3 | Implementation is delayed to FY2029 (FY2030) for states whose FY2025 (FY2026) rate × 1.5 reaches 20% | 7 U.S.C. 2013(a)(2)(B)(iii) |
| A4 | QC active-case annual minimum sample: 300 for N≤10,000; 300+0.042(N−10,000) capped at 2,400; reduced option 300 / 300+0.0153(N−12,941) capped 1,020. Negative: 150 / 150+0.144(N−500) capped 800; reduced 150 / 150+0.1224(N−683) capped 680 | 7 C.F.R. 275.11(b); FY2024 technical documentation pp. (formulas restated) |
...
| # | Fact | Source |
|---|---|---|
| C1 | Seven states' encoded SNAP benefit computations reproduce the QC file exactly at zero tolerance: CO 856, NY 847, CA 883, AZ 922, GA 945, MD 722, TX 906 = 6,081 cases; six asserted values per case (gross income, standard deduction, shelter deduction, net income, maximum allotment, benefit) | axiom-oracles PRs #244, #268, #269; committed suite reports |
...
| # | Fact | Source |
|---|---|---|
| D1 | Colorado FY2024: 856 sampled cases, 305 with payment errors (STATUS 2/3); $112.6M/yr weighted error dollars on $1.268B issuance (8.88% file-derived; official regression-adjusted rate 9.97% = 7.91 over + 2.06 under) | amterr lab ANALYSIS (snapshot); snap-fy24QC-PER.pdf |
| D2 | QC cause coding: strict computation classes (programming/arithmetic/mass change) carry $8.2M/yr (7.3% of error dollars); adding policy-misapplied, budgeted-wrong, computer-user reaches $11.9M (10.5%). National: $320M/yr (3.9%) strict, $1.5B/yr (18.4%) broad, of $8.1B/yr weighted error dollars | amterr lab ANALYSIS layer 1 |
| D3 | Finding-nature classification: pure-math 3.3%, input-with-system-cause 6.6%, mixed 4.2%, other input 86.0% of error dollars | amterr lab ANALYSIS layer 2 |
| D4 | Replay: reconstructing pre-edit original values (public $3-shift solver, FY2024 adaptation) and replaying through the verified engine explains 246/283 filtered error cases (86.9%) as correct arithmetic on wrong facts; solver and engine partition the 283 cases identically; 37 cases form the computation-side upper bound | amterr lab ANALYSIS layer 3; reconstruct_co_fy2024.R |
...
| E3 | Distributional model: nine conditional quantiles of log|D|, per-case monotone, exponential-in-logs tail (scale 0.4674) fitted on top-decile OOF residuals; weighted FY2024 quantile coverage within 3pp at 7 of 9 levels (q.75 −3.5pp, q.90 −3.3pp) | distributional_results.json; FINDINGS.md |
...
| E6 | Model-vs-bootstrap simulated measured rate (official-centering convention): the model's raw level underpredicts high-error states (NY model mean 11.02% vs bootstrap 14.10%; CO 8.62 vs 9.98) — the app therefore anchors levels on the model's own baseline mean | distributional_results.json simulation_validation; FINDINGS.md |
...
| # | Fact | Source |
|---|---|---|
| F1 | Tier assignment near boundaries is noisy at regulatory sample sizes: P(a fresh QC-style sample lands in a different tier than the official point rate implies) — ND 49%, WA 48%, CO 47%, KS 47%, NV 47% | app data pipeline (scripts_build_data.py + simulate); mc lab results (snapshot JSON) |
| F2 | Colorado: official 9.97% (0.03pp below the 15% boundary), $1.27B issuance, expected FY2028-rule cost $156M/yr, SD $33M; the 10%→15% tier step is ≈ $63M/yr | mc lab results; app all-states table |
| F3 | Audit volume is a two-sided lever: +500 reviews raises expected cost for just-above-boundary states (CA −$29.5M, PA −$13.3M, TX −$13.2M expectation) and lowers it for just-below states (MO +$4.0M, TN +$3.7M, IN +$3.3M); it nearly always cuts the SD of the bill (CA −$43M) | mc lab results §2 |
| F4 | Category-suppression accounting bound: removing 50% (100%) of observed error dollars in the four named simplification categories is worth ≈$606M/yr (≈$1.31B/yr) nationally in expected cost share | mc lab results §1 |
...
- Cross-state correlation 0.535 ⇒ r²≈0.29 — never "about half of variance".
