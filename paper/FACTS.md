...
## O. The official-vs-file wedge and the full-universe roadmap (added in revision 6)

| # | Fact | Source |
|---|---|---|
| O1 | Wedge = official FY2024 rate − file-computable FY2024 weighted rate (federal re-review integration + ineligible-case error the file never records): median 31% of the official rate across the 53 jurisdictions, range −3.4% (NV; MN also negative — the adjustment can run downward) to 81% (AK); CO 2.55pp of 9.97 (25.6%), NY 6.25pp of 14.09 (44.4%). Anchoring holds the level right but the layer contributes no sampling variance and no scenario reaches it; disclosed per state in the app's adoption panel and the method notes | tests/test_adoption_numbers.py::test_wedge_disclosure_stats_match_the_app_and_paper (computed from committed data.json) |
| O2 | Full-universe end state (analysis/MICROSIM_ROADMAP.md): the engine computes what should happen (deterministic layer), an estimation layer on the Microcosm caseload models what does happen, error = the gap; QC files become calibration/validation, the wedge becomes model output calibrated to official-rate components, and measured-rate simulation becomes a QC-style sample drawn FROM the universe. Critical path: the error model's distributional validation failures; then QC-grade attribute imputation onto Microcosm units; then per-state official-component calibration targets | analysis/MICROSIM_ROADMAP.md |
